...
public class DefaultMessageListenerContainer extends AbstractPollingMessageListenerContainer {
    ...
    private class AsyncMessageListenerInvoker implements SchedulingAwareRunnable {
        ...
        public void run() {
            ...
            try {
                /**
                 *  根据每个任务设置的最大处理消息数量maxMessagesPerTask而作不同处理
                 *
                 *  小于0默认为无限制，一直接收消息
                 */
        ...
        /**
         * 如果按照正常的流程其实是不会进入while循环中的，而是直接进入函数invokeListenerO来接收消息并激活监听器，
         * 但是，不可能让循环一直持续下去，我们要考虑到暂停线程或者恢复线程的情况，这时，isRunning0函数就派上用场了。
         * <p>
         * isRunning()用来检测标志位this.running 状态进而判断是否需要进入while循环。由于要维护当前线程激活数量，
         * 所以引入了wasWaiting变量，用来判断线程是否处于等待状态。如果线程首次进入等待状态，则需要减少线程来激活数量计数器。
         * <p>
         * 当然，还有一个地方需要提一下，就是线程等待不是一味地采用while循环来控制，因为如果单纯地采用while循环会浪费CPU的时钟周期，
         * 给资源造成巨大的浪费。这里，Spring采用全局控制变量lifecycleMonitor的wait0方法来暂停线程，
         * 所以，如果终止线程需要再次恢复的话，除了更改this.running 标志位外，
         * 还需要调用lifecyclelMonitor.notify 或者lifecycle Monitor.notifyAll来使线程恢复。
         *
         * @return
         * @throws JMSException
         */
