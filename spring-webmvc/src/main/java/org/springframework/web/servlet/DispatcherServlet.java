...
public class DispatcherServlet extends FrameworkServlet {
     ...
     * 默认情况下，Spring是没有配置multipart处理的，如果需要则在Web应用的上下文中添加multipart解析器。
     * 常用配置如下：
     ...
     * 1. 通过URL参数配置国际化 -- AcceptHeaderLocaleResolver
     * 根据 HTTP头部中的Accept-Language(注意大小写)值来解析locale，并且是不可变的。
     * 配置如下:
     * <bean id="localeResolver" class="org.springframework.web.servlet.i18n.AcceptHeaderLocaleResolver"/>
     * <p>
     * 2. 基于session配置国际化 -- SessionLocaleResolver
     * 它通过检验用户会话中预置的属性来解析区域。最常用的是根据用户本次会话过程中的语言设定决定语言种类(例如用户登录时选择语言种类，则此次登录周期内统一使用此语言设定)，
     * 如果该会话属性不存在，它会根据 HTTP头部Accept-Language确定默认区域，
     * 配置如下:
     * <bean id="localeResolver" class="org.springframework.web.servlet.i18n.SessionLocaleResolver"/>
     * <p>
     * 3. 基于cookie配置国际化 -- CookieLocaleResolver
     * 用于通过浏览器的cookie设置取得Locale对象。这种策略在应用程序不支持会话或者状态必须保存在客户端时有用，
     * 配置如下:
     * <bean id="localeResolver" class="org.springframework.web.servlet.i18n.CookieLocaleResolver"/>
     * <p>
     * 4. 如果需要根据用户请求来改变语言，Spring提供了一个已经实现的拦截器：ThemeChangelnterceptor，配置如下：
     ...
     * 不同的用户使用不同的主题资源
     * <p>
     * 对于主题解析器的子类主要有3个比较常用的实现。
     * 以主题文件summer.properties为例。
     * <p>
     * 1. FixedThemeResolver 用于选择一个固定的主题，配置如下：
     * <bean id="themeResolver" class="org.springframework.web.servlet.theme.FixedThemeResolver">
     * <property name="defaultThemeName" value="summer"/>
     * </bean>
     * <p>
     * 2. CookieThemeResolver 用于实现用户所选的主题，以cookie的形式存放在客户端的机器上，配置如下：
     * <bean id="themeResolver" class="org.springframework.web.servlet.theme.CookieThemeResolver">
     * <property name="defaultThemeName" value="summer"/>
     * </bean>
     * <p>
     * 3. SessionThemeResolver 用于主题保存在用户的HTTP Session中，配置如下：
     * <bean id="themeResolver" class="org.springframework.web.servlet.theme.SessionThemeResolver">
     * <property name="defaultThemeName" value="summer"/>
     * </bean>
     * <p>
     * 4. AbstractThemeResolver是一个抽象类,被 SessionThemeResolver和FixedThemeResolver继承，用户也可以继承它来自定义主题解析器。
     * <p>
     * 5. 如果需要根据用户请求来改变主题，Spring提供了一个已经实现的拦截器：ThemeChangelnterceptor，配置如下：
    ...
    protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            try {
                ...
                /**
                 * 根据当前的handler寻找对应的HandlerAdapter,通过会是SimpleControllerHandlerAdapter
                 */
                ...
                if (isGet || "HEAD".equals(method)) {
                    /**
                     * Spring Last-Modified缓存机制: 如果服务端内容无变化，只需要返回304状态码
                     * 1.在客户端第一次输入URL时，服务器端会返回内容和状态码200，表示请求成功，同时会添加一个“Last-Modified”的响应头，
                     * 表示此文件在服务器上的最后更新时间，例如“Last-Modified:Wed，14 Mar 201210：22：42 GMT”表示最后更新时间为。
                     *
                     * 2.客户端第二次请求此URL时，客户端会向服务器发送请求头“If-Modified-Since”，询问服务器该时间之后当前请求内容是否有被修改过，
                     * 如“If-Modified-Since:Wed，14 Mar 201210：22：42GMT”，
                     * 如果服务器端的内容没有变化，则自动返回HTTP304状态码(只要响应头，内容为空，这样就节省了网络带宽)。
                     *
                     * 通过实现 org.springframework.web.servlet.mvc.LastModified
                     */
                ...
                /**
                 * 真正的激活handler并返回视图,通过会是SimpleControllerHandlerAdapter
                 */
    ...
    protected View resolveViewName(String viewName, @Nullable Map<String, Object> model,
        ...
        /**
         * 解析视图名称，以 org.springframework.web.servlet.view.InternalResourceViewResolver 为例
         */
        if (this.viewResolvers != null) {
