...
public class RmiClientInterceptor extends RemoteInvocationBasedAccessor
    ...
    public void prepare() throws RemoteLookupFailureException {
        ...
        /**
         * 如果lookupStubOnStartup设置为true，那么获取stub的工作就会在系统启动时被执行并缓存，从而提高使用时候的响应时间。
         */
     ...
     * 1.使用自定义的套接字工厂。如果使用这种方式，你需要在构造Registry实例时将自定义套接字工厂传入，
    ...
    protected Remote lookupStub() throws RemoteLookupFailureException {
        try {
            ...
            if (this.registryClientSocketFactory != null) {
                ...
                /**
                 * 与之前服务端的套接字工厂类似，这里的registryClientSocketFactory用来连接RMI服务器，
                 * 用户通过实现 RmiClientSocketFactory 接口来控制用于连接的socket的各种参数。
                 */
     ...
     * 1.获取的stub是RmiInvocationHandler类型的，从服务端获取的stub是RmiInvocationHandler，
     * 就意味着服务端也同样使用了Spring去构建，那么自然会使用Spring中作的约定，进行客户端调用处理。Spring中的处理方式被委托给了dolnvoke方法。
     * <p>
     * 2.当获取的stub不是RminvocationHandler类型，那么服务端构建RMI服务可能是通过普通的方法或者借助于Spring外的第三方插件，
     * 那么处理方式自然会按照RMI中普通的处理方式进行，而这种普通的处理方式无非是反射。因为在invocation中包含了所需要调用的方法的各种信息，
     * 包括方法名称以及参数等，而调用的实体正是stub，那么通过反射方法完全可以激活stub中的远程调用。
     *
     * @param invocation
     * @param stub
     * @return
