...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
    ...
    public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition) throws TransactionException {
        ...
        // PROPAGATION_REQUIRED、PROPAGATION_REQUIRES_NEW、PROPAGATION_NESTED都需要新建事务
            ...
            try {
                ...
                /**
                 * 构造transaction，包括设置 ConnectionHolder、隔离级别、timout
                 * 如果是新连接，绑定到当前线程
                 */
    ...
    private TransactionStatus handleExistingTransaction(
        ...
        /**
         * PROPAGATION_REQUIRES_NEW表示当前方法必须在它自己的事务里运行，一个新的事务将被启动，如果有一个事务正在运行的话，则在这个方法运行期间被挂起。
         * 而Spring中对于此种传播方式的处理与新事务建立最大的不同点在于使用suspend方法将原事务挂起。
         *
         * 对于挂起操作的主要目的是记录原有事务的状态，为了在当前事务执行完毕后在将原事务还原
         */
        // 创建一个新的事务，并挂起当前的事务
        ...
        /**
         * PROPAGATION_NESTED表示如果当前正有一个事务在运行中，则该方法应该运行在一个嵌套的事务中，被嵌套的事务可以独立于封装事务进行提交或者回滚，
         * 如果封装事务不存在，行为就像PROPAGATION_REQUIRES_NEW。对于嵌入式事务的处理，Spring中主要考虑了两种方式的处理。
         */
        // 嵌入式事务处理
        if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NESTED) {
            ...
            // Spring中允许嵌入事务的时候，则首选设置保存点的方式作为嵌入式事务异常处理的回滚
    ...
    /**
     * 某个事务是另一个事务的嵌入事务，但是，这些事务又不在Spring的管理范围内，或者无法设置保存点，
     * 那么Spring会通过设置回滚标识的方式来禁止提交。首先当某个嵌入事务发生回滚的时候会设置回滚标识，
                ...
                if (status.hasSavepoint()) {
                    ...
                    if (status.hasTransaction()) {
                        if (status.isLocalRollbackOnly() || isGlobalRollbackOnParticipationFailure()) {
                            ...
                            //如果当前事务不是独立的事务，那么只能标记状态，等到事务链执行完毕后统一回滚，多数用于JTA
